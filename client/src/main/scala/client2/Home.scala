...
object Home:
  ...
  val refreshFeedsBus: EventBus[Unit] =
    new EventBus
  val markAllAsReadBus: EventBus[Unit] =
    new EventBus
  ...
  given feedDecoder: Decoder[FeedItemData] =
    deriveDecoder
  ...
  private val itemClickObserver =
    Observer[Try[List[Long]]] {
      case Success(ids) =>
        feedVar.update(xs =>
          xs.map { elem =>
  ...
  private val feedsObserver =
    Observer[List[FeedItemData]](feedVar.set(_))
    ...
    onMountBind(ctx =>
      ...
      markAllAsReadBus --> { _ =>
        ...
        if (ids.nonEmpty) {
          ...
          response.addObserver(itemClickObserver)(
            ctx.owner
          )
        }
      }
  ...
  private def feeds(): Element =
    ...
    UList(
      ...
      children <-- feedSignal.split(_.id)(
        renderItem
      ),
      data --> feedsObserver,
    ...
    div(
      ...
      Card(
        _.slots.header := CardHeader(
          _.slots.avatar := Icon(
            _.name := IconName.feed
          ),
          _.titleText <-- itemSignal.map(_.title),
          _.subtitleText <-- itemSignal.map(
            _.channelTitle
          ),
        ...
        UList(
          ...
          _.events.onItemClick
            .map(
              _.detail.item.dataset.get("feedId")
            )
            .map(id => List(id.get))
            .flatMapStream(
              updateFeedRequest(_)
          ...
          child <-- itemSignal.map(x =>
            CustomListItem(
              ...
              dataAttr("feed-id") := x.id
                .toString(),
              dataAttr("seen") := x.read
                .toString()
            )
          )
        )
  ...
  private def unsafeParseToHtmlFragment(
    html: String
  ): HtmlElement =
    div(
      DomApi
  ...
  private def getFeedsRequest(): EventStream[Try[List[FeedItemData]]] =
    FetchStream
      .withDecoder(
        responseDecoder[List[FeedItemData]]
      )
      .get(s"${HOST}/api/feed/all")
      .mapSuccess(_.get)

  private def updateFeedRequest(
    ids: List[String]
  ): EventStream[Try[List[Long]]] =
    val seen = feedSignal
      .now()
      .filter(ids.contains(_))
      .filter(!_.read)
