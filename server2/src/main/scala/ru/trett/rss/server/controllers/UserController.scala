...
object UserController {
    ...
    def routes(userService: UserService, cacheUpdater: User => IO[Unit])(using LoggerFactory[IO]): AuthedRoutes[User, IO] =
        ...
        AuthedRoutes.of {
            ...
            case req @ POST -> Root / "api" / "user" / "settings" as user =>
                for {
                    ...
                    updatedUser = user.copy(settings = User.Settings(settings.retentionDays, settings.hideRead))
                    result <- userService.updateUserSettings(updatedUser)
                    _ <- logger.info(s"User: ${user.email} was updated with settings: ${user.settings}")
